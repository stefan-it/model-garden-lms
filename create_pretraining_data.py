...
"""Create masked LM/next sentence masked_lm TF examples for BERT/TEAMS with various modifications"""
...
flags.DEFINE_string("tokenizer_model_id", None, "Model id on HF Model Hub.")

flags.DEFINE_enum(
    "packing_strategy",
    "original",
    ["original", "best-fit", "roberta"],
    "Specified the to be used packing strategy. 'original' uses the BERT packing strategy, "
    "whereas 'best-fit' uses Best-fit packing as proposed in the "
    "'Fewer Truncations Improve Language Modeling' (Ding et al., 2024) paper. "
    "'roberta' refers to the 'FULL-SENTENCES' approach from the RoBERTa paper.",
)
...
def write_instance_to_example_files(
    ...
    total_subwords_with_padding = 0
    total_subwords_without_padding = 0
    ...
    for inst_index, instance in enumerate(instances):
        ...
        total_written += 1
        total_subwords_with_padding += max_seq_length
        total_subwords_without_padding += len(instance.tokens)

        if inst_index < 20:
            logging.info("*** Example ***")
            logging.info(
                "tokens: %s",
                " ".join([x for x in instance.tokens]),
            )
            ...
            logging.info("number padding tokens: %s", str(max_seq_length - len(instance.tokens)))

    for writer in writers:
        writer.close()

    with open(f"{output_files[0]}_stats.jsonl", "wt") as f_out:
        stats = {
            "total_subwords_with_padding": total_subwords_with_padding,
            "total_subwords_without_padding": total_subwords_without_padding,
            "total_instances": total_written,
        }
...
def create_training_instances(
    ...
    logging.info("⛓️ Start tokenizing input file into documents...")
    ...
    # We do not shuffle here, because all documents came randomly from our datatrove pipeline
    #rng.shuffle(all_documents)

    logging.info("✂️  Finished tokenizing input files! Now starting with generating training examples...")

    vocab_words = list(dict(sorted(tokenizer.vocab.items(), key=lambda x: x[1])).keys())
    instances = []
    ...
    logging.info("🔄 Start shuffling training examples...")
    rng.shuffle(instances)

    logging.info("✔️  Finished shuffling training examples!")

    return instances


def create_training_instances_best_fit(
    input_files,
    tokenizer,
    processor_text_fn,
    max_seq_length,
    ...
    for _ in range(dupe_factor):
        instances.extend(
            create_instances_from_documents_best_fit(
                all_documents,
                max_seq_length,
                short_seq_prob,
                masked_lm_prob,
    ...
    logging.info("Shuffling %d instances...", len(instances))
    rng.shuffle(instances)
    logging.info("Shuffling %d instances completed!", len(instances))
    return instances


def create_training_instances_roberta(
    input_files,
    tokenizer,
    processor_text_fn,
    max_seq_length,
    ...
    for _ in range(dupe_factor):
        instances.extend(
            create_instances_from_documents_roberta(
                all_tokens,
                max_seq_length,
                short_seq_prob,
                masked_lm_prob,
...
def split_list_into_chunks(input_list, chunk_length):
    for i in range(0, len(input_list), chunk_length):
        yield input_list[i : i + chunk_length]


def pack_documents_original(tokenized_documents, max_num_tokens):
    ...
    def find_bin(doc):
        for b in bins:
            if sum(len(d) for d in b) + len(doc) <= max_num_tokens:
                return b
        return None
...
def pack_documents(tokenized_documents, max_num_tokens):
    len_idx_tokenized_documents = []

    logging.info("🧮 [Best-Fit]: Start building helper structure with document length and its index...")

    for idx, tokenized_document in enumerate(tokenized_documents):
        len_idx_tokenized_documents.append([len(tokenized_document), idx])

    # Old solution:
    # sorted_docs = sorted(len_idx_tokenized_documents, reverse=True)

    logging.info("🧮 [Best-Fit]: Start sorting documents by their length...")

    len_idx_tokenized_documents.sort(key=itemgetter(0), reverse=True)
    sorted_docs = len_idx_tokenized_documents
    ...
    def find_bin(doc):
        for b in bins:
            if sum(d[0] for d in b) + doc[0] <= max_num_tokens:
                return b
        return None

    logging.info("🧮 [Best-Fit]: Start building bins with packaged documents...")
    ...
    logging.info("🧮 [Best-Fit]: Completed building bins with packaged documents!")

    # Return results
    return bins


def create_instances_from_documents_best_fit(
    all_documents,
    max_seq_length,
    short_seq_prob,
    masked_lm_prob,
    ...
    tokenized_documents_splitted = []

    logging.info("Splitting %d documents into %d chunks...", len(all_documents), max_num_tokens)

    for tokenized_document in all_documents:
        flattened_document = list(itertools.chain(*tokenized_document))
        if len(flattened_document) > max_num_tokens:
            splitted_chunks = list(split_list_into_chunks(flattened_document, max_num_tokens))

            for splitted_chunk in splitted_chunks:
                tokenized_documents_splitted.append(splitted_chunk)
        else:
            tokenized_documents_splitted.append(flattened_document)

    logging.info("Packing %d documents...", len(tokenized_documents_splitted))

    packed_documents = pack_documents(tokenized_documents_splitted, max_num_tokens)

    logging.info("Creating training instances for %d documents...", len(packed_documents))

    for packed_document in packed_documents:
        a_end = 1

        current_chunk = []

        # flattened_packed_document = list(itertools.chain(*packed_document))
        real_document = []
        for document in packed_document:
            _, idx = document
            real_document.append(tokenized_documents_splitted[idx])

        flattened_packed_document = list(itertools.chain(*real_document))

        for token in flattened_packed_document:
            current_chunk.append(token)

        assert len(current_chunk) <= max_num_tokens
        ...
        instance = TrainingInstance(
            ...
            is_random_next=len(packed_document) > 1,
            masked_lm_positions=masked_lm_positions,
            masked_lm_labels=masked_lm_labels,
        )
        instances.append(instance)

    logging.info("Creating training instances for %d documents completed!", len(packed_documents))

    return instances


def create_instances_from_documents_roberta(
    all_tokens,
    max_seq_length,
    short_seq_prob,
    masked_lm_prob,
    ...
    # We implement the RoBERTa-style like "FULL-SENTENCES" method, see section 4.2 of RoBERTa paper
    instances = []

    # Account for [CLS], [SEP], [SEP]
    max_num_tokens = max_seq_length - 3

    logging.info("Perform creation of training instances via RoBERTa 'FULL-SENTENCES' strategy...")

    chunk_span = 0
    while chunk_span < len(all_tokens):
        current_chunk = all_tokens[chunk_span : chunk_span + max_num_tokens]
        ...
        instance = TrainingInstance(
            tokens=tokens,
            segment_ids=segment_ids,
            is_random_next=len(all_tokens) > 1,
            masked_lm_positions=masked_lm_positions,
            masked_lm_labels=masked_lm_labels,
        )
        instances.append(instance)

        chunk_span += max_num_tokens

    logging.info("Creation of training instances completed!")
...
def get_processor_text_fn():
    def processor_text_fn(text):
        # This is necessary, because the input file is read-in as binary!
...
def main(_):
    tokenizer = AutoTokenizer.from_pretrained(FLAGS.tokenizer_model_id)
    processor_text_fn = get_processor_text_fn()
    ...
    if FLAGS.packing_strategy == "best-fit":
        instances = create_training_instances_best_fit(
            input_files,
            tokenizer,
            processor_text_fn,
    ...
    elif FLAGS.packing_strategy == "original":
    ...
    elif FLAGS.packing_strategy == "roberta":
        instances = create_training_instances_roberta(
            input_files,
            tokenizer,
            processor_text_fn,
            FLAGS.max_seq_length,
...
if __name__ == "__main__":
    ...
    flags.mark_flag_as_required("tokenizer_model_id")
    app.run(main)
